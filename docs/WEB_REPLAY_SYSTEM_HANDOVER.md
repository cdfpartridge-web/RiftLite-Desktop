...
> **Current-pointer notice (2026-08-30):** This is an architecture/privacy reference. For current version, repository, worktree, and deployment truth, start with [`docs/START_HERE.md`](./START_HERE.md) and [`docs/HANDOVER-2026-08-30.md`](./HANDOVER-2026-08-30.md). Release-baseline statements below are historical unless the current handover confirms them.

Architecture content last deeply verified: 2026-07-12; release and replay-finalization notes updated 2026-08-09
Historical architecture baseline: RiftLite `v0.8.03` (package SemVer `0.8.3`); published desktop release at the last deep update: `v0.9.42`
...
From v0.9.42, **Review later** closes after its pending match row is durably stored. Replay and Web Replay finalization then continue in a non-blocking background lane instead of holding the modal through upload/network deadlines. Prepared work remains retryable; confirming the match re-arms it as blocking work, while deleting the review retains the existing durable and runtime fences so an in-flight finalizer cannot recreate it.
...
The local v0.9.42 candidate adds two fullscreen layers without changing replay data or remounting the embedded guest. The desktop Web Replay page can occupy the full application display while keeping its selected replay and exact frame. Inside the hosted player, a labelled player-only control plus `F` / `Esc` uses the Fullscreen API and tracks `fullscreenchange`, so browser and Electron labels stay accurate. The replay partition already permits the narrow fullscreen permission; no broader navigation, media, or IPC permission was added.
