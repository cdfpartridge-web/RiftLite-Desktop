...
function installRestrictedEmbeddedPermissions(
  ...
  const exactRequester = (requestingContents: WebContents | null, requestingUrl: string, isMainFrame: boolean, permission: string) => (
    requestingContents?.id === webContents.id &&
    !webContents.isDestroyed() &&
    isMainFrame &&
    isAllowedEmbeddedPermission(policy, requestingUrl || webContents.getURL(), permission, permissions)
  );
  embeddedSession.setPermissionCheckHandler((requestingContents, permission, _origin, details) => (
    exactRequester(requestingContents, details.requestingUrl || "", details.isMainFrame, String(permission))
  ));
...
function secureGameWebContents(webContents: WebContents, policy: Extract<EmbeddedWebviewPolicy, { kind: "game" }>): void {
  ...
  webContents.setWindowOpenHandler(({ url }) => {
    if (policy.platform === "atlas" && isAtlasDeckPageUrl(url)) {
      // Keep target=_blank deck links in the same persistent Atlas guest too.
      void webContents.loadURL(url).catch((error) => logStartupIssue("Atlas deck navigation failed", error));
      return { action: "deny" };
    }
    if (!isAllowedGamePopupNavigation(policy, url)) {
  ...
  webContents.on("will-prevent-unload", (event) => {
    if (policy.platform !== "atlas") return;
    handleAtlasDeckBeforeUnload(event, webContents.getURL(), () => {
      if (!mainWindow || mainWindow.isDestroyed()) return false;
      return dialog.showMessageBoxSync(mainWindow, {
        type: "question",
        title: "Unsaved deck changes",
        message: "Leave without saving your deck changes?",
        detail: "Atlas has unsaved changes. Stay to save them, or leave and discard those changes.",
